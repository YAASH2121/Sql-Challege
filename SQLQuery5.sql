...
Select * from Shows As S join Hosts As H on S.HostID=H.HostID where H.HostName='Vrajesh Hirjee'

Select S.ShowName,COUNT(A.AwardID) AS TotalAward from Awards AS A join Shows AS S on A.ShowID=S.ShowID  Group by S.ShowName

select ShowName,LaunchDate,PartnerName from Shows join ShowPartnerships As S on Shows.ShowID=S.ShowID join Partnerships As P on S.PartnershipID=P.PartnershipID where P.PartnerName='Spotify'

select * from Hosts as H where JoinDate < '2010-01-01'

select ShowName,LaunchDate  from Shows as S order by LaunchDate Desc

Select HostName,COUNT(H.ShowCount) from Hosts as H group by HostName

Select O.PlatformName,O.Link from OnlinePresence  as O 

select * from Stations where Location='South' and LaunchDate > '2010-01-01'
